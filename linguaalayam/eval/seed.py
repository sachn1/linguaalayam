...
_DEFINITIONS: dict[str, list[tuple[str, str]]] = {
    ...
    "love":       [("n", "സ്നേഹം; പ്രണയം; ആഴമായ വൈകാരിക ബന്ധം"),
                   ("v", "സ്നേഹിക്കുക")],
    ...
    "light":      [("n", "വെളിച്ചം; പ്രകാശം; ദ്യുതി"),
                   ("adj", "ഭാരം കുറഞ്ഞ; ലഘുവായ")],
    "hope":       [("n", "പ്രത്യാശ; ആശ; ഭാവിയെക്കുറിച്ചുള്ള പ്രതീക്ഷ"),
                   ("v", "പ്രത്യാശിക്കുക")],
    ...
    "melancholy": [("n", "വി‌ഷ‌ദ‌ം; ദുഃ‌ഖ‌ം; ഉ‌ൾ‌പ്പൊ‌ള്ളൽ‌"),
                   ("adj", "ദുഃ‌ഖ‌ക‌ര‌മായ; നി‌ര‌ദ്ര‌നായ")],
}


def _build_entries() -> list[EnMlEntry]:
    dataset = load_dataset(Path(__file__).resolve().parent.parent.parent / "data/eval/queries.jsonl")
