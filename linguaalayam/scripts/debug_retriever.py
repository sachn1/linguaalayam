...
def _load_config(overrides: list[str]):
    """Load and compose the full Hydra config without the @hydra.main decorator."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--corpus", default="all", help="Corpus config to use (default: all)"
    )
    parser.add_argument(
        "--embedding", default="model", help="Embedding config to use"
    )
